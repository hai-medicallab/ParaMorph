...
from Models.PMFN_Cross_up5 import TransMatch
...
def train():
    ...
    # 日志文件
    log_name = "PMFN_Cross_up5_LPBA_"+str(args.n_iter) + "_" + str(args.lr) + "_" + str(args.alpha)
    ...
    # Training loop.
    for epoch in range(iterEpoch, args.n_iter + 1):
        ...
        for input_moving, fig_name in input_moving_all:
            ...
            adjust_learning_rate(opt, epoch, args.n_iter, args.lr)
            ...
            print("%d, %s, %f, %f, %f" % (epoch, fig_name, loss.item(), sim_loss.item(), grad_loss.item()), file=f)
        ...
        for file in test_file_lst:
            ...
            # 计算DSC
            dice = compute_label_dice(fixed_label[0, 0, ...].cpu().detach().numpy(), pred_label[0, 0, ...].cpu().detach().numpy())
            print("{0}" .format(dice))
            DSC.append(dice)
        ...
        save_checkpoint({
            ...
            }, save_dir='/data/nizehao_data/PMFN_Cross_up5_lpba40/', filename='dsc{:.4f}epoch{:0>3d}.pth.tar'.format(np.mean(DSC), epoch+1))
